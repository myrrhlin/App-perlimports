...
# whether to output json (or else console text)
has json => (
    is      => 'ro',
    isa     => Bool,
    lazy    => 1,
...
#  Carp => ['croak', ..], ...
# but will be altered after processing, to reflect what we think
...
sub _build_possible_imports {
    ...
    for my $word ( @{$before} ) {
        ...
        if (my $stm = $word->statement) {
            ...
            if ($stm->isa('PPI::Statement::Package')) {
                ...
                if (my $packname = $stm->module) {
                    ...
                    my $packword = $stm->schild(1); # package bareword
                    next if $word == $packword;
                }
        ...
        next if $const{ $word } && is_function_call($word);
...
# but with each include that gets processed, this list also gets updated, even 
# in lint mode. We do this so that we can keep track of what previous modules
# are really importing, avoiding both duplicate imports and duplicate errors.
...
INCLUDE:
    foreach my $include ( $self->all_includes ) {
        ...
        if ( !$inserted ) {
            ...
            # Regardless of lint mode, we (try to) edit the document (the
            # _original_imports attribute specifically) to contain the
            # statement text we think are needed.
    ...
    if ($self->lint) {
        ...
        if (@unknown) {
            ...
            $self->_warn_line_for_linter('does not appear to be defined', $_) for @unknown;
        }
    }
...
sub _warn_line_for_linter {
    ...
    my $name = $elem->isa('PPI::Token::Quote') ? $elem->string
    ...
    if ( $self->json ) {
        ...
        my $justification = sprintf(
            '❌ |%s| (%s) at %s line %i',
            $name, $reason, $self->_filename, $elem->line_number
        );
        $self->logger->error($justification);
        if ($stm) {
            my $line = sprintf( "%04u %s", $stm->line_number, $stm->content );
